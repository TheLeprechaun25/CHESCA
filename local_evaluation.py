    ...
    try:
        while True:
            ...
            if not done:
                ...
                metrics_dict = env.evaluate_citylearn_challenge()
                episode_metrics.append(metrics_dict)
                print(f"Episode complete: {episodes_completed}")

                for i in range(len(metrics_dict)):
                    key = list(metrics_dict.keys())[i]
                    value = list(metrics_dict.values())[i]['value']
...
if __name__ == '__main__':
    class Config:
        data_dir = './data/'
        # Change the SCHEMA path to the path of the schema you want to test. See data/schemas/ for available schemas
